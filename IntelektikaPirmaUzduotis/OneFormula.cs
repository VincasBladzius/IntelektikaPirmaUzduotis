...
namespace IntelektikaPirmaUzduotis
{
    public class OneFormula
    {
        ...
        public List<List<object>> CalculateDistances(string[][] sample, string[][] data, int point)
        {
            ...
            if (sample == null)
            {
                MessageBox.Show("Nepasirinkote Imties");
                Environment.Exit(0);
            }
            double sum;
            ...
            {
                sum = Math.Pow(Convert.ToInt32(data[point][0]) - Convert.ToInt32(sample[i][0]), 2) + Math.Pow(Convert.ToInt32(data[point][1]) - Convert.ToInt32(sample[i][1]), 2);
                sum = Math.Sqrt(sum);
